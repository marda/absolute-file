...
class FileCRUDManager extends BaseManager
{
    ...
    public function createFromBase64($base64, $fileName, $path, $type = "", $name = "", $isRndName = true)
    {
        $image = Image::fromString(base64_decode($base64));
        if (!$image)
        {
            return false;
        }
        ...
        $image->save(WWW_DIR . $path . $rndFileName,80,Image::PNG);
        return $this->database->table('file')->insert(
                        array(
                            'name' => $name,
                            'type' => $type,
